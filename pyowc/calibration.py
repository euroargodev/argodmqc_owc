...
from .helper import (
    ...
    process_profiles_grid_variables,
    process_profiles_la_variables,
    remove_statical_outliers,
    selected_historical_points,
    sort_numpy_array,
)
...
def update_salinity_mapping(float_dir, config, float_name):
    ...
    filename = os.path.sep.join(
        [config["FLOAT_SOURCE_DIRECTORY"], float_dir, float_name + config["FLOAT_SOURCE_POSTFIX"]]
    )
    ...
    for i in range(0, missing_profile_index.__len__()):
        ...
        if (
            not np.isnan(float_data["float_long"])
            and not np.isnan(float_data["float_lat"])
            ...
            float_elev, float_x, float_y = get_topo_grid(
                float_long_tbase - 1,
                float_long_tbase + 1,
            ...
            float_interp = interpolate.RegularGridInterpolator(
                (float_y[:, 0], float_x[0, :]), float_elev, method="linear"
            )
            ...
            if grid_lat.__len__() > 0:
                ...
                index = find_besthist(
                    grid_data["grid_lat"],
                    grid_data["grid_long"],
                    grid_data["grid_dates"],
                    grid_data["grid_z"],
                    float_data["float_lat"],
                    float_long_0,
                    float_data["float_date"],
                    float_data["float_z"],
                    lat_large,
                    lat_small,
                    long_large,
                ...
                for n_level in range(float_level_count):
                    ...
                    # that aren't NaN's
                    if (
                        not np.isnan(float_data["float_sal"][n_level])
                        and float_data["float_pres"][n_level] >= map_p_exclude
                    ):
                        hist_data = process_profile_hist_variables(
                            grid_data=best_hist_data,
                        ...
                        if (np.unique(hist_long_flatten).size > 1 or np.unique(hist_lat_flatten).size > 1) and hist_sal_flatten.size > 5:
                            ...
                            float_data_array = np.array(
                                [
                                    float_data["float_lat"],
                                    float_data["float_long"],
                                    float_data["float_date"],
                                    float_data["float_z"],
                                ]
                            ).reshape((-1, 4))

                            hist_data_array = np.column_stack(
                                (
                                    hist_data["hist_lat"],
                                    hist_data["hist_long"],
                                    hist_data["hist_dates"],
                                    hist_data["hist_z"],
                                )
                            )

                            mapped_values = map_data_grid(
                                hist_sal_flatten,
                                float_data_array,
                                hist_data_array,
                                long_large,
                            ...
                            mapped_residuals = map_data_grid(
                                sal_residual,
                                float_data_array,
                                hist_data_array,
                                long_small,
                            ...
                            data["la_mapped_sal"][n_level, profile_index] = mapped_values[0] + mapped_residuals[0]

                            dot_map_values = np.dot(mapped_values[1], mapped_values[1])
                            data["la_map_sal_errors"][n_level, profile_index] = np.sqrt(
                                dot_map_values + mapped_residuals[1] * mapped_residuals[1]
                            )
    ...
    data = sort_numpy_array(
        data, sorted_profile_index, ["la_ptmp", "la_mapped_sal", "la_map_sal_errors", "la_noise_sal", "la_signal_sal"]
    )
    ...
    save_location = os.path.sep.join(
        [config["FLOAT_MAPPED_DIRECTORY"], config["FLOAT_MAPPED_PREFIX"] + float_name + config["FLOAT_MAPPED_POSTFIX"]]
    )
...
def calc_piecewisefit(float_dir, float_name, system_config):
    ...
    float_source_data_path = os.path.sep.join(
    ...
    float_mapped_data_path = os.path.sep.join(
        [
            system_config["FLOAT_MAPPED_DIRECTORY"],
            float_dir,
            system_config["FLOAT_MAPPED_PREFIX"] + float_name + system_config["FLOAT_MAPPED_POSTFIX"],
        ]
    )
    ...
    float_calseries_path = os.path.sep.join(
        [
            ...
            float_dir,
            system_config["FLOAT_CALSERIES_PREFIX"] + float_name + system_config["FLOAT_MAPPED_POSTFIX"],
        ]
    )
    ...
    if n_seq == 1 and max_breaks.__len__() > 1:
        ...
        print(str(max_breaks), " specified, should be ", str([max_breaks.__len__(), n_seq]))
    ...
    # we have multiple cal series, make sure that break information is provideed for all segments
        ...
        # error in specification of max breaks
            ...
            print(str(max_breaks), " specified, should be 1 or ", str([max_breaks.__len__(), n_seq]))
            sstatus = 0
    ...
    if breaks.__len__() > 0:
        ...
        for n in range(n_seq):
            ...
            if nb > max_breaks[n]:
                ...
                print("Specified ", str(nb), " breaks. Will search up to ", str(max_breaks[n]), " breaks")
    ...
    if sstatus == 0:
        float_calib_filename = os.path.sep.join(
            [
                system_config["FLOAT_CALIB_DIRECTORY"],
                float_dir,
                system_config["FLOAT_CALIB_PREFIX"] + float_name + system_config["FLOAT_CALIB_POSTFIX"],
            ]
        )

        savemat(
            float_calib_filename,
            {
                "cal_SAL": cal_sal,
    ...
    for i_seq in range(n_seq):
        ...
        theta, p, index, var_s_th, th = find_10thetas(
            copy.deepcopy(unique_sal),
        ...
        if pp.__len__() > 0:
            ...
            icond = gsw.conversions.C_from_SP(ten_sal, ten_ptmp, 0)
            mapped_cond = gsw.conversions.C_from_SP(ten_mapped_sal, ten_ptmp, 0)

            mapped_cond1 = gsw.conversions.C_from_SP(ten_mapped_sal + ten_mapsalerrors / 100, ten_ptmp, 0)
            ...
            if pcond_factor[0][calindex].__len__() > 0:
                ...
                cal_sal[:, calindex] = gsw.conversions.SP_from_C(cal_cond[:, calindex], unique_ptmp_1, 0)
                cal_cond_err[:, calindex] = np.dot(np.ones((m, 1)), pcond_factor_err[:, calindex]) * unique_cond
                cal_sal1[:, calindex] = gsw.conversions.SP_from_C(
                    (cal_cond[:, calindex] + cal_cond_err[:, calindex]), unique_ptmp, 0
                )
