...
class CalSalCurve(unittest.TestCase):
    ...
    def test_plot_runs(self, mockshow):
        ...
        cal_data_path = os.path.sep.join(
            [
        ...
        float_data_path = os.path.sep.join(
            [
                TESTS_CONFIG["FLOAT_SOURCE_DIRECTORY"],
                TESTS_CONFIG["TEST_FLOAT_SOURCE"] + TESTS_CONFIG["FLOAT_SOURCE_POSTFIX"],
            ]
        )
...
class SalVar(unittest.TestCase):
    ...
    def test_plot_runs(self, mockshow):
        ...
        grid_data_path = os.path.sep.join(
            [
                TESTS_CONFIG["TEST_DIRECTORY"],
                "float_mapped_test",
        ...
        self.assertEqual(
            plots.sal_var_plot(
                ...
                copy.deepcopy(pres),
                copy.deepcopy(theta),
                copy.deepcopy(grid_sal),
                copy.deepcopy(grid_errors),
                copy.deepcopy(grid_ptmp),
                copy.deepcopy(cal_sal),
                copy.deepcopy(cal_sal_errors),
                no_boundaries,
                profile_no,
                float_name,
                config,
...
class TS(unittest.TestCase):
    ...
    def test_plot_runs(self, mockshow):
        ...
        thetas = find_10thetas(
            copy.deepcopy(sal), copy.deepcopy(ptmp), copy.deepcopy(pres), copy.deepcopy(grid_ptmp), [], [], [], [], 0.5
        )
...
class ThetaSal(unittest.TestCase):
    ...
    def test_plot_runs(self, mockshow):
        ...
        thetas = find_10thetas(
            copy.deepcopy(sal), copy.deepcopy(theta), copy.deepcopy(pres), copy.deepcopy(map_ptmp), [], [], [], [], 0.5
        )
