...
class GetData(unittest.TestCase):
    ...
    def test_removes_argo_float(self):
        ...
        self.assertTrue(
            data_normal["long"].__len__() - data_removed["long"].__len__() == self.config["TEST_FLOAT_N_TO_REMOVE"],
    ...
    def test_returns_correct_shape(self):
        ...
        self.assertTrue(
            this_data["sal"].shape == this_data["ptmp"].shape == this_data["temp"].shape,
            "Ocean characteristic data should be the same shape",
        )
        self.assertTrue(
            this_data["long"].shape == this_data["lat"].shape == this_data["dates"].shape,
            "Spatial/temporal data should be the same shape",
        )
        self.assertTrue(
            this_data["sal"].shape[1] == this_data["long"].shape[0], "Should be a profile for every location"
        )
...
class GetRegionData(unittest.TestCase):
    ...
    def setUp(self):
        self.float_name = TESTS_CONFIG["TEST_FLOAT_SOURCE"]
        self.config = TESTS_CONFIG
        self.wmo_boxes = np.array(
            [[self.config["TEST_FLOAT_WMO_BOXES"][0], 1, 1, 1], [self.config["TEST_FLOAT_WMO_BOXES"][1], 1, 1, 1]]
        )
    ...
    def test_return_shape(self):
        ...
        self.assertTrue(
            test["grid_sal"].shape == test["grid_ptmp"].shape == test["grid_pres"].shape,
            "salinity, pressure, and potential temperature " "arrays should be same shape",
        )
        self.assertTrue(
            test["grid_lat"].shape == test["grid_long"].shape == test["grid_dates"].shape,
            "longitude, latitude, and date arrays should be the same shape",
        )
        self.assertTrue(
            test["grid_sal"].shape[1]
            == test["grid_ptmp"].shape[1]
            == test["grid_pres"].shape[1]
            == test["grid_lat"].shape[0]
            == test["grid_long"].shape[0]
            == test["grid_dates"].shape[0]
            == self.index.__len__(),
    ...
    def test_gets_different_data(self):
        ...
        test_ctd = get_region_data(
            np.array([[self.config["TEST_FLOAT_WMO_BOXES"][0], 1, 0, 0]]),
            self.float_name,
            self.config,
            self.index,
            self.pres,
        )
        test_bot = get_region_data(
            np.array([[self.config["TEST_FLOAT_WMO_BOXES"][0], 0, 1, 0]]),
            self.float_name,
            self.config,
            self.index,
            self.pres,
        )
        test_argo = get_region_data(
            np.array([[self.config["TEST_FLOAT_WMO_BOXES"][0], 0, 0, 1]]),
            self.float_name,
            self.config,
            self.index,
            self.pres,
        ...
        self.assertTrue(
            test_ctd["grid_sal"].shape[1] != test_argo["grid_sal"].shape[1],
            "Should get a different data set, if we have specified it",
        )
        self.assertTrue(
            test_bot["grid_sal"].shape[1] != test_argo["grid_sal"].shape[1],
            "Should get a different data set, if we have specified it",
        )
    ...
    def test_should_only_get_specific_indices(self):
        ...
        test_one = get_region_data(self.wmo_boxes, self.float_name, self.config, [50], self.pres)
    ...
    def test_raise_exception_bad_indices(self):
        ...
        with self.assertRaises(Exception) as no_index:
            get_region_data(self.wmo_boxes, self.float_name, self.config, [], self.pres)

        self.assertTrue("NO DATA FOUND" in str(no_index.exception))

        with self.assertRaises(Exception) as big_index:
            get_region_data(self.wmo_boxes, self.float_name, self.config, [99999999999999999], self.pres)
...
class GetRegionHistLoc(unittest.TestCase):
    ...
    def test_returns_three(self):
        ...
        self.assertEqual(lat.__len__(), long.__len__(), "Should have equal numbers of latitude and longitude")
        self.assertEqual(lat.__len__(), age.__len__(), "should have equal number of spatial and temporal data")
    ...
    def test_current_float_removed(self):
        ...
        lat_rem, long_rem, age_rem = get_region_hist_locations(wmo_box_argo, self.float_name, self.config)
        # All data:
        lat_no_rem, long_no_rem, age_no_rem = get_region_hist_locations(wmo_box_argo, "DUMMY_WMO", self.config)

        self.assertNotEqual(lat_rem.__len__(), lat_no_rem.__len__(), "Did not remove current argo float")

        self.assertGreater(
            long_no_rem.__len__(), long_rem.__len__(), "Should have more values for the data with no argo removal"
        )
    ...
    def test_can_choose_data(self):
        ...
        for i in range(0, data_argo.__len__()):
            self.assertNotEqual(
                data_argo[i].__len__(), data_bot[i].__len__(), "Should have different data for argo and bottle"
            )
            self.assertNotEqual(
                data_argo[i].__len__(), data_ctd[i].__len__(), "Should have different data for argo and ctd"
            )
            # no bottle database reference,
            self.assertNotEqual(
                data_bot[i].__len__(), data_ctd[i].__len__(), "Should have different data for bottle and ctd"
            )
    ...
    def test_can_combine_data(self):
        ...
        for data in all_data:
            self.assertEqual(
                data.__len__(), self.config["TEST_FLOAT_N_DATA"], "Should get the same amount of data as matlab"
            )
    ...
    def test_can_combine_boxes(self):
        ...
        half_data = get_region_hist_locations(
            [self.wmo_boxes[0], [self.config["TEST_FLOAT_WMO_BOXES"][1], 0, 0, 0]], "none", self.config
        )

        for i in range(0, all_data.__len__()):
            self.assertGreater(
                all_data[i].__len__(), some_data[i].__len__(), "Should have more data when using multiple boxes"
            )
            self.assertEqual(
                some_data[i].__len__(),
                half_data[i].__len__(),
                "Should get equal data with using one ox and using " "two boxes, but getting no data from second box",
            )
