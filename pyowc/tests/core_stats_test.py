...
class BrkPtFit(unittest.TestCase):
    ...
    def test_needs_no_weights(self):
        ...
        for i in range(residual.__len__()):
            self.assertAlmostEqual(residual[i], residual_ans[i], 12, "matlab parameters should match python ones")
    ...
    def test_runs_with_weights(self):
        ...
        fit_param_ans = np.array([1.20259478507721, -0.587941247050749])
        residual_ans = np.array(
            [
                0.741576274102326,
                1.01478102614965,
    ...
    def test_returns_correct_elements(self):
        ...
        for i in range(0, expected_size[0]):
            for j in range(0, expected_size[1]):
                self.assertAlmostEqual(test[i, j], expected[i, j], 15, "covariance matrix is incorrect")
...
class Covarxytpv(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.points1 = (
            np.array(
                [
                    [-0.057996, 0.053195, 1.9740875, 5.229838],
                    [-0.0564902, 0.0631170, 1.9870367, 4.6300392],
                    [-0.05208, 0.0619770, 1.9941118, 4.6536932],
                ]
            )
            * 10**3
        )
    ...
    def test_returns_zeroes_for_extremely_different_value(self):
        ...
        for i in range(0, covar.__len__()):
            for j in range(0, covar[i].__len__()):
                self.assertAlmostEqual(covar[i][j] * 1000000, expected[i][j] * 1000000, "covariance isn't just 1's")
    ...
    def test_returns_expected_answers(self):
        ...
        expected = np.array(
            [
                [1, 0.001350185414046, 0.001712995003897],
        ...
        for i in range(0, covar.__len__()):
            for j in range(0, covar[i].__len__()):
                self.assertAlmostEqual(covar[i][j], expected[i][j], 15, "covariances is not as expected")
    ...
    def test_returns_expected_answers_regression(self):
        ...
        expected = np.array(
            [
                [0.985163822020838, 0.986423041063023, 0.999577585526622],
        ...
        inp1 = np.array(
            [
                [0.34097335, 0.48557151, 0.672727, 0.98830669],
        ...
        inp2 = np.array(
            [
                [0.3487758740, 3.92049930e-04, 0.366825482, 0.460229042],
    ...
    def test_allows_1_dimensional_data(self):
        ...
        for i in range(covar.__len__()):
            self.assertAlmostEqual(covar[0][i], expected_ans[i], 15, "unexpected answer with one dimensional data")
...
class FitCond(unittest.TestCase):
    ...
    def test_max_no_breaks(self):
        ...
        python_test = fit_cond(self.in_x, self.in_y, self.in_err, self.in_cov, "max_no_breaks", 4)

        for i in range(python_test[0].__len__()):
            self.assertEqual(python_test[0][i], self.xfit[i], "xfit is incorrect")

        for i in range(python_test[2][0].__len__()):
            self.assertAlmostEqual(python_test[2][0, i], 0.000122830414419824, 12, "slope error is incorrect")

        for i in range(python_test[5].shape[0]):
            for j in range(python_test[5].shape[1]):
                if ~np.isnan(python_test[5][i, j]):
                    self.assertAlmostEqual(python_test[5][i, j], self.sta_mean[i, j], 12, "mean is incorrect")

        for i in range(python_test[6].shape[0]):
            for j in range(python_test[6].shape[1]):
                if ~np.isnan(python_test[6][i, j]):
                    self.assertAlmostEqual(python_test[6][i, j], self.sta_rms[i, j], 12, "rms is incorrect")
