...
from ..utilities import change_dates, wrap_longitude
...
def get_region_data(pa_wmo_numbers, pa_float_name, config, index, pa_float_pres):
                    ...
                    for i in range(0, data_length):
                        ...
                        if good_indices.__len__() > 0:
                            ...
                            if new_depth > max_depth != 0:
                                grid_pres = np.append(
                                    grid_pres, np.ones((how_many_cols, new_depth - max_depth)) * np.nan, axis=1
                                ).reshape((how_many_cols, how_many_rows))
                                grid_ptmp = np.append(
                                    grid_ptmp, np.ones((how_many_cols, new_depth - max_depth)) * np.nan, axis=1
                                ).reshape((how_many_cols, how_many_rows))
                                grid_sal = np.append(
                                    grid_sal, np.ones((how_many_cols, new_depth - max_depth)) * np.nan, axis=1
                                ).reshape((how_many_cols, how_many_rows))
    ...
    for column in range(grid_sal.shape[1]):
        if np.all(np.isnan(grid_sal[:, column - nans])) or np.all(np.isnan(grid_pres[:, column - nans])):
    ...
    if nans > 0:
        print("Warning: found ", nans, " all NaNs in your dataset. These water columns have been removed")
...
def frontal_constraint_saf(config, grid_data, float_data):
    ...
    if float_lat < -30:
        ...
        if (
            not np.argwhere(np.diff(float_pres[isok].T).size == 0)
            and (isok.__len__() > 2)
            ...
            if crit_saf == 0:
                sal_int = np.interp(
                    deph.flatten(), float_pres[isok].flatten(), float_sal[isok].flatten(), left=np.nan, right=np.nan
                )
                temp_int = np.interp(
                    deph.flatten(), float_pres[isok].flatten(), float_tmp[isok].flatten(), left=np.nan, right=np.nan
                )

                northinf_int = np.interp(
                    temp_int.flatten(),
                    np.flip((t_mean_n - t_std_n).flatten()),
                    np.flip((s_mean_n - s_std_n).flatten()),
                    left=np.nan,
                    right=np.nan,
                )
                northsup_int = np.interp(
                    temp_int.flatten(),
                    np.flip((t_mean_n + t_std_n).flatten()),
                    np.flip((s_mean_n + s_std_n).flatten()),
                    left=np.nan,
                ...
                southinf_int = np.interp(
                    sal_int.flatten(),
                    (s_mean_s - s_std_s).flatten(),
                    (t_mean_s - t_std_s).flatten(),
                    left=np.nan,
                    right=np.nan,
                )
                southsup_int = np.interp(
                    sal_int.flatten(),
                    (s_mean_s + s_std_s).flatten(),
                    (t_mean_s + t_std_s).flatten(),
                    left=np.nan,
                ...
                inte_idx = (
                    ~np.isnan(sal_int)
                    & ~np.isnan(southinf_int)
                    & ~np.isnan(southsup_int)
                    & ~np.isnan(northinf_int)
                    & ~np.isnan(northsup_int)
                )
        ...
        if crit_saf != 0:
            for i in range(grid_long.__len__()):
                ...
                if (
                    not np.argwhere(np.diff(grid_pres[isok, i]) == 0)
                    and (isok.__len__() > 2)
                    and (np.min(grid_pres[isok, i]) < 300 < np.max(grid_pres[isok, i]))
                ):
    ...
    return {
        "grid_sal": best_hist_sal,
        "grid_ptmp": best_hist_ptmp,
        "grid_pres": best_hist_pres,
        "grid_lat": best_hist_lat,
        "grid_long": best_hist_lon,
        "grid_dates": best_hist_dates,
        "grid_z": best_hist_z,
    }
